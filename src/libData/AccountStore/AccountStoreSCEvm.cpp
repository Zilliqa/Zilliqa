...
namespace evm {
    ...
    // Define it as a const because we need to set a view for the boundaries with the same name
    const char *EVM_HISTOGRAM = "zilliqa.evm.histogram";

    zil::metrics::doubleHistogram_t &GetHistogramCounter() {
        static auto histogram = Metrics::GetMeter()->CreateDoubleHistogram(EVM_HISTOGRAM, "evm latency histogram",
                                                                           "ms");
        return histogram;
    }
...
void AccountStoreSC::EvmCallRunner(const INVOKE_TYPE /*invoke_type*/,  //
    ...
    const auto worker = [&span, &args, &ret, &result]() -> void {
        try {
            ...
            Metrics::GetInstance().CaptureEMT(span,
                                              zil::metrics::FilterClass::ACCOUNTSTORE_EVM,
                                              zil::trace::FilterClass::ACC_EVM,
                                              evm::GetInvocationsCounter(),
                                              ss.str());
...
bool AccountStoreSC::UpdateAccountsEvm(const uint64_t &blockNum,
    ...
    switch (evmContext.GetContractType()) {
        case Transaction::CONTRACT_CREATION: {
            ...
            if (fromAccountBalance <
                ...
                std::stringstream ss;
                ss << "The account doesn't have enough gas to create a contract : " << gasDepositWei;
                Metrics::GetInstance().CaptureEMT(span,
                                                  zil::metrics::FilterClass::ACCOUNTSTORE_EVM,
                                                  zil::trace::FilterClass::ACC_EVM,
                                                  evm::GetInvocationsCounter(),
    ...
    span->SetStatus(opentelemetry::trace::StatusCode::kOk, "Executing contract finished OK");
