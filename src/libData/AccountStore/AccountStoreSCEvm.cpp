    ...
    switch (evmContext.GetContractType()) {
        case Transaction::CONTRACT_CREATION: {
            ...
            if (fromAccountBalance <
                ...
                auto constexpr str = "The account doesn't have enough gas to create a contract";
                LOG_GENERAL(WARNING,str);
            ...
            if (evmContext.GetCode().empty()) {
                constexpr auto str = "Creating a contract with empty code is not feasible.";
                LOG_GENERAL(WARNING,str);
                TRACE_ATTRIBUTE msg{{"From",   fromAddr.hex()},
                                    {"reason", str}};
                TRACE_EVENT(span, ACC_EVM, "return", msg);
            ...
            if (evm_call_run_succeeded) {
                ...
                auto constexpr str = "Executing contract Creation transaction finished unsuccessfully";
            ...
            if (evmContext.GetTransaction().GetGasLimitZil() < gasRemainedCore) {
                ...
                error_code = TxnStatus::INSUFFICIENT_GAS_LIMIT;
                TRACE_ATTRIBUTE msg{{"From",   fromAddr.hex()}, {"reason", "Gas Underflow"}};
                TRACE_EVENT(span, ACC_EVM, "return", msg);
                return false;
            }
        ...
        case Transaction::CONTRACT_CALL: {
            ...
            if (contractAccount == nullptr) {
                ...
                LOG_GENERAL(INFO, str);
                TRACE_ATTRIBUTE msg{{"Address",  evmContext.GetTransaction().GetToAddr().hex()}, {"reason", str}};
                TRACE_EVENT(span, ACC_EVM, "return", msg);
                error_code = TxnStatus::INVALID_FROM_ACCOUNT;
                return false;
            }
            ...
            if (contractAccount->GetCode().empty()) {
                error_code = TxnStatus::NOT_PRESENT;
                auto constexpr str = "Trying to call a smart contract that has no code will fail";
                LOG_GENERAL(WARNING, str);
                TRACE_ATTRIBUTE msg{{"Address",  evmContext.GetTransaction().GetToAddr().hex()}, {"reason", str}};
                TRACE_EVENT(span, ACC_EVM, "return", msg);
                return false;
