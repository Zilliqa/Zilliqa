...
if (zil::metrics::Filter::GetInstance().Enabled(\
    ...
    Metrics::GetInstance().CaptureEMT(\
        span, zil::metrics::FilterClass::ACCOUNTSTORE_EVM, zil::trace::FilterClass::ACC_EVM, \
        evm::GetInvocationsCounter(), MSG);\
}
...
#define LOCAL_INCREMENT_CALLS_COUNTER(PARAMETER_KEY, PARAMETER_VALUE) \
    INCREMENT_CALLS_COUNTER( evm::GetInvocationsCounter(),ACCOUNTSTORE_EVM, PARAMETER_KEY , PARAMETER_VALUE );
