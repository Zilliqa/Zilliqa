    ...
    namespace local {
        ...
        Z_DBLHIST &GetEvmLatency() {
            ...
            static Z_DBLHIST counter{Z_FL::ACCOUNTSTORE_HISTOGRAMS,
                                     "processing", latencieBoudaries,
                                     "latency of processing", "ms"};
            return counter;
        }
...
bool AccountStore::UpdateAccountsTemp(
    ...
    if (ENABLE_EVM == false && isEvm) {
        LOG_GENERAL(WARNING, "EVM is disabled so not processing this EVM transaction ");
    ...
    if (delay > 0) {
        if (zil::local::GetEvmLatency().Enabled()) {
            zil::local::GetEvmLatency().Record((delay / 1000.0), {{status ? "passed" : "failed", __FUNCTION__}});
        }
    }
    return status;
}
