...
// Commitment is composed of a random secret scalar, a public point and a hash of the public point.
...
struct CommitHashPoint : public Serializable {
  /// The hash value.
  ...
  CommitHashPoint(const CommitPoint& point);

  /// Constructor for loading existing hash from a byte stream.
  CommitHashPoint(const std::vector<unsigned char>& src, unsigned int offset);

  /// Copy constructor.
  CommitHashPoint(const CommitHashPoint& src);
  ...
  /// Assignment operator.
  CommitHashPoint& operator=(const CommitHashPoint& src);

  /// Equality comparison operator.
  bool operator==(const CommitHashPoint& r) const;
};
