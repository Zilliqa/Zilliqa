...
// bool MultiSig::SignResponse(const CommitSecret & commitSecret, const
// Challenge & challenge, const PrivKey & privkey, Signature & result)
// {
//     LOG_MARKER();
...
//         // Compute s = k - r*krpiv
//         // r*kpriv
//         err = (BN_mod_mul(result.m_s.get(), challenge.m_c.get(),
//         privkey.m_d.get(), m_curve.m_order.get(), ctx.get()) == 0); if (err)
//         {
...
//         // k-r*kpriv
//         err = (BN_mod_sub(result.m_s.get(), commitSecret.m_s.get(),
//         result.m_s.get(), m_curve.m_order.get(), ctx.get()) == 0); if (err)
//         {
