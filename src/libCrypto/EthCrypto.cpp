...
zbytes RecoverEip2930Transaction(zbytes transaction, int expectedChainId) {
  ...
  LOG_GENERAL(WARNING, "recovering key of " << DataConversion::Uint8VecToHexStrRet(transaction));
  ...
  LOG_GENERAL(WARNING, "recreated: " << DataConversion::Uint8VecToHexStrRet(messageRecreatedBytes));

  auto prehash = ethash::keccak256(messageRecreatedBytes.data(), messageRecreatedBytes.size());

  return DerivePubkey(fields.signature, signatureYParity, &prehash.bytes[0]);
}
...
zbytes GetOriginalHash(TransactionCoreInfo const& info, uint64_t chainId) {
  ...
  switch (version) {
    ...
    case TRANSACTION_VERSION_ETH_EIP_2930: {
      ...
      LOG_GENERAL(WARNING, "before hashing, the signed value is " << DataConversion::Uint8VecToHexStrRet(messageRecreatedBytes));

      auto prehash = ethash::keccak256(messageRecreatedBytes.data(), messageRecreatedBytes.size());
      return zbytes{&prehash.bytes[0], &prehash.bytes[32]};
    }
