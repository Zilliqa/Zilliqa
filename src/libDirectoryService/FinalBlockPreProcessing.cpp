...
bool DirectoryService::WaitUntilCompleteFinalBlockIsReady() {
  ...
  int timeout_time = (int)DS_TX_PROCESSING_TIMEOUT;
  LOG_GENERAL(INFO,
...
bool DirectoryService::RunConsensusOnFinalBlockWhenDSPrimary() {
  ...
  if (!ComposeFinalBlock()) {
    LOG_EPOCH(WARNING, m_mediator.m_currentEpochNum,
              "DS ComposeFinalBlock (pre-prep) failed");
    return false;
  }
...
void DirectoryService::RunConsensusOnFinalBlock() {
...

  {
    ...
    // We've been already here, don't do the same thing again
