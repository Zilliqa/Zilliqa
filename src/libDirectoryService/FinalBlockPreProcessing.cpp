...
bool DirectoryService::CheckBlockTypeIsFinal()
{
    ...
    {
        LOG_GENERAL(WARNING, "Type check failed. Expected: "
...
bool DirectoryService::CheckMicroBlockHashRoot()
{
    ...
    {
        LOG_GENERAL(WARNING, "Microblock root hash in proposed final block by "
                    "leader is incorrect");
        return false;
    }
...
bool DirectoryService::CheckIsMicroBlockEmpty()
{
    ...
    for (unsigned int i = 0; i < txRootHashesInMicroBlocks.size(); i++)
    {
        ...
        for (auto& microBlock : m_microBlocks)
        {
            LOG_GENERAL(INFO, "Checking " << microBlock.GetHeader().GetTxRootHash());
                ...
                == txRootHashesInMicroBlocks[i])
            {
                ...
                {
                    LOG_GENERAL(WARNING, "IsMicroBlockEmpty in proposed final "
                                "block is incorrect "
                                << i << " Expected: "
                                << (microBlock.GetHeader().GetNumTxs() == 0)
                                << " Received: "
...
void DirectoryService::RunConsensusOnFinalBlock()
{
    ...
    if (m_mode == PRIMARY_DS)
    {
        ...
        {
            LOG_GENERAL(INFO, "Throwing exception after "
