...
bool DirectoryService::RunConsensusOnDSBlockWhenDSPrimary() {
  ...
  // TODO: Revise DS block structure
  {
    ...
    m_pendingDSBlock.reset(new DSBlock(
        DSBlockHeader(
            dsDifficulty, difficulty, prevHash, m_mediator.m_selfKey.second,
            blockNum, m_mediator.m_currentEpochNum,
            static_cast<uint64_t>(get_time_as_int()), m_mediator.m_curSWInfo,
