                                                   ...
                                                   uint32_t shardId)
{
    ...
    const vector<bool>& headerSigBitmap = microBlock.GetHeaderSigBitmap();
    if (shard.size() != headerSigBitmap.size())
    {
        LOG_MESSAGE("Error: Mismatch: Shard size = "
                    << shard.size()
                    << ", co-sig bitmap size = " << headerSigBitmap.size());
        return false;
    }
    ...
    for (auto& kv : shard)
    {
        if (headerSigBitmap.at(index) == true)
        {
    ...
    if (count != ConsensusCommon::NumForConsensus(headerSigBitmap.size()))
    {
    ...
    if (Schnorr::GetInstance().Verify(message, 0,
                                      MicroBlockHeader::HEADER_SIZE_NEEDED,
                                      microBlock.GetHeaderSig(), *aggregatedKey)
        == false)
    {
    ...
    const vector<unsigned char>& message, unsigned int offset, const Peer& from)
{
    ...
    {
        LOG_MESSAGE2(to_string(m_mediator.m_currentEpochNum).c_str(),
                     "Error: Microblock co-sig verification failed");
        return false;
    }
