...
void Lookup::SetLookupNodes() {
  ...
  LOG_GENERAL(INFO, "Before locking");
  ...
  LOG_GENERAL(INFO, "After locking");

  m_startedTxnBatchThread = false;
  m_lookupNodesOffline.clear();

  LOG_GENERAL(INFO, "Before update nodes");
  ...
  LOG_GENERAL(INFO, "After update nodes");
  ...
  for (const auto& lookup : m_lookupNodes) {
    ...
    if (isMul) {
      LOG_GENERAL(INFO, "Erasing multiplier");
      m_multipliers.erase(mulItr);
    }
  ...
  LOG_GENERAL(INFO, "Lookup size: " << m_lookupNodes.size());
  for (const auto& l : m_lookupNodes) {
    LOG_GENERAL(INFO, "Lookup: " << l.first << ", " << l.second);
  }
  LOG_GENERAL(INFO, "Multiplier size: " << m_multipliers.size());
  for (const auto& m : m_multipliers) {
    LOG_GENERAL(INFO, "Multiplier: " << m.first << ", " << m.second);
  }
  ...
  LOG_GENERAL(INFO, "m_lookupNodesStatic set to new m_lookupNodes");
  /*
  ...
  */
  if (USE_REMOTE_TXN_CREATOR) {
    LOG_GENERAL(INFO, "USE_REMOTE_TXN_CREATOR");
    ...
    LOG_GENERAL(INFO, "Genesis wallet size: " << GENESIS_WALLETS.size());
