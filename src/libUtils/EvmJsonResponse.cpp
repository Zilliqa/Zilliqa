...
namespace evmproj {
    ...
    evmproj::CallRespose& GetReturn(const Json::Value& oldJason,
        ...
        try {
            ...
            std::cout << "Error parsing json from evmds " << e.what()
                      << std::endl;
            return fo;
        }

        std::cout << "Response from EVM-DS " << std::endl << newJason
                  << std::endl;
        ...
        try {
                ...
                if (node.key() == "apply" && node.value().is_array()) {
                    for (const auto& ap: node.value()) {
                        for (const auto& map: ap.items()) {
                            ...
                            try {
                                ...
                                std::cout << "address : " << e.what()
                                          << std::endl;
                            }
                            try {
                                fo.m_apply.m_balance = arr["balance"];
                            } catch (std::exception& e) {
                                std::cout << "balance : " << e.what()
                                          << std::endl;
                            }
                            nlohmann::json cobj;
                            try {
                    ...
                    for (const auto& er: node.value().items()) {
                        try {
                            ...
                            std::cout << "Exception caught on exit_reason"
                                      << std::endl;
                        }
                    }
            ...
            std::cout << "exception caught in parsing EVM-DS return value : " << e.what() << std::endl;
        }
        return fo;
    }
    ...
    std::ostream&
    operator<<(std::ostream& os, evmproj::ApplyInstructions& evm) {
        ...
        os << "reset_storage : " << std::boolalpha << evm.m_resetStorage
           << std::endl;
