...
void WebsocketServer::removeHdl(const string& ip, WEBSOCKETQUERY q_enum) {
  IpSocketMap::iterator it;
  switch (q_enum) {
    case NEWBLOCK: {
      lock_guard<mutex> g(m_mutexTxBlockSockets);
...
void WebsocketServer::removeHdl(const std::string& remote) {
  WEBSOCKETQUERY q_enum;
  {
    lock_guard<mutex> g(m_mutexEqIndex);
    auto find = m_eqIndex.find(remote);
    if (find == m_eqIndex.end()) {
      LOG_GENERAL(WARNING, "removeHdl for " << remote << " failed");
      return;
    }
    q_enum = find->second;
...
void WebsocketServer::on_message(const connection_hdl& hdl,
  ...
  websocketpp::lib::error_code ec;
  m_server.send(hdl, std::move(query), msg->get_opcode(), ec);
  if (ec) {
