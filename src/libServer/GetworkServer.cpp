...
ethash_mining_result_t GetWorkServer::VerifySubmit(const string& nonce,
  ...
  if (HexStringToUint64(nonce, &winning_nonce)) {
    ...
    auto success =
        POW::GetInstance().PoWVerify(m_curWork.blocknum, m_curWork.difficulty,
                                     header, mixdigest, winning_nonce);
    if (!success) {
      LOG_GENERAL(WARNING, "PoW Verify Failed!");
      return FAIL_RESULT;
    }

    auto hash_result = POW::GetInstance().LightHash(
        m_curWork.blocknum, POW::StringToBlockhash(header), winning_nonce);

    auto calced_mix_digest = POW::BlockhashToHexString(hash_result.mix_hash);
    if (mixdigest != calced_mix_digest) {
      LOG_GENERAL(WARNING, "mixdigest mismatch: " << mixdigest << " != "
                                                  << calced_mix_digest);
      return FAIL_RESULT;
    }

    return ethash_mining_result_t{
        POW::BlockhashToHexString(hash_result.final_hash), mixdigest,
        winning_nonce, true};
  }
