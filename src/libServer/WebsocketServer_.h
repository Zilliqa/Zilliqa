...
#ifndef ZILLIQA_SRC_LIBSERVER_WEBSOCKETSERVER__H_
#define ZILLIQA_SRC_LIBSERVER_WEBSOCKETSERVER__H_
...
struct eventlogsubscription : public subscription {
...
struct hash<websocketpp::connection_hdl> {
  size_t operator()(websocketpp::connection_hdl const& hdl) const noexcept {
    std::size_t seed = 0;
    boost::hash_combine(seed, hdl.lock().get());
    return seed;
  }
};
}  // namespace std
...
class WebsocketServer : public Singleton<WebsocketServer> {
  std::unordered_map<std::string, websocketpp::connection_hdl> m_hdls;
  std::unordered_map<
      websocketpp::connection_hdl,
      std::unordered_map<WEBSOCKETQUERY, std::unique_ptr<subscription>,
 ...
 private:
  ...
  static void removeHdl(const std::string& remote);

  /// remove a connection_hdl in memory via endpoint ip and WEBSOCKETQUERY
  static void removeHdl(const std::string& ip, WEBSOCKETQUERY q_enum);
...
#endif  // ZILLIQA_SRC_LIBSERVER_WEBSOCKETSERVER__H_