...
//
//    LOG_GENERAL(WARNING, "Throwing due to revert!!! " << response.ReturnedBytes());
...
std::string EthRpcMethods::GetEthEstimateGas(const Json::Value& json) {
  ...
  if (!json.isMember("from")) {
    ...
    auto const temp1 = json["from"].asString();
    auto const asBytes = DataConversion::HexStrToUint8VecRet(temp1);
    fromAddr = Address{asBytes};
  }
  ...
  // transaction yet.
  auto const fromAddrTmp = DataConversion::Uint8VecToHexStrRet(fromAddr.asBytes());
  txEnv->SetSource(fromAddrTmp);
