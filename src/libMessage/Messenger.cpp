...
namespace
{
        ...
        const uint16_t leaderID, const pair<PrivKey, PubKey>& leaderKey)
    {
        ...
        announcement.mutable_info()->set_consensusid(consensusID);
        announcement.mutable_info()->set_blockhash(blockHash.data(),
                                                   blockHash.size());
        announcement.mutable_info()->set_leaderid(leaderID);

        if (announcement.info().IsInitialized() == false)
        {
            LOG_GENERAL(
                WARNING,
        ...
        {
            tmp.resize(announcement.info().ByteSize()
                       + announcement.dsblock().ByteSize());
            announcement.info().SerializeToArray(
                tmp.data(), announcement.info().ByteSize());
        ...
        const uint16_t leaderID, const PubKey& leaderKey)
    {
        ...
        if (announcement.info().consensusid() != consensusID)
        {
            LOG_GENERAL(WARNING,
                        "Consensus ID mismatch. Expected: "
                            << consensusID << " Actual: "
                            << announcement.info().consensusid());
            return false;
        }

        if ((announcement.info().blockhash().size() != blockHash.size())
            || !equal(blockHash.begin(), blockHash.end(),
                      announcement.info().blockhash().begin()))
        {
            LOG_GENERAL(WARNING, "Block hash mismatch.");
            return false;
        }

        if (announcement.info().leaderid() != leaderID)
        {
            LOG_GENERAL(WARNING,
                        "Leader ID mismatch. Expected: "
                            << leaderID
                            << " Actual: " << announcement.info().leaderid());
            return false;
        }
        ...
        if (Schnorr::GetInstance().Verify(tmp, signature, leaderKey) == false)
        {
...
bool Messenger::SetDSDSBlockAnnouncement(
    ...
    const vector<map<PubKey, Peer>>& shards, const vector<Peer>& dsReceivers,
    ...
    vector<unsigned char>& messageToCosign)
{
    ...
    if (dsblock->IsInitialized() == false)
    {
...
bool Messenger::GetDSDSBlockAnnouncement(
    ...
    Peer& powWinnerPeer, vector<map<PubKey, Peer>>& shards,
    ...
    vector<vector<Peer>>& shardSenders, vector<unsigned char>& messageToCosign)
{
    ...
    if (announcement.IsInitialized() == false)
    {
        LOG_GENERAL(WARNING, "ConsensusAnnouncement initialization failed.");
        return false;
    }

    if (announcement.has_dsblock() == false)
    {
        LOG_GENERAL(WARNING, "DSDSBlockAnnouncement initialization failed.");
        return false;
    }
    ...
    const TxBlock& txBlock, vector<unsigned char>& messageToCosign)
{
    ...
    if (finalblock->IsInitialized() == false)
    {
    ...
    vector<unsigned char>& messageToCosign)
{
    ...
    if (announcement.has_finalblock() == false)
    {
        LOG_GENERAL(WARNING, "DSFinalBlockAnnouncement initialization failed.");
        return false;
    }
    ...
    const VCBlock& vcBlock, vector<unsigned char>& messageToCosign)
{
    ...
    if (vcblock->IsInitialized() == false)
    {
    ...
    vector<unsigned char>& messageToCosign)
{
    ...
    if (announcement.has_vcblock() == false)
    {
        LOG_GENERAL(WARNING, "DSVCBlockAnnouncement initialization failed.");
        return false;
    }
    ...
    const MicroBlock& microBlock, vector<unsigned char>& messageToCosign)
{
    ...
    if (microblock->IsInitialized() == false)
    {
        LOG_GENERAL(WARNING,
    ...
    vector<unsigned char>& messageToCosign)
{
    ...
    if (announcement.has_microblock() == false)
    {
        LOG_GENERAL(WARNING,
                    "NodeMicroBlockAnnouncement initialization failed.");
        return false;
                                   ...
                                   const pair<PrivKey, PubKey>& backupKey)
{
    ...
    if (result.data().IsInitialized() == false)
    {
    ...
    const pair<PrivKey, PubKey>& leaderKey)
{
    ...
    SerializableToProtobufByteArray(challenge,
                                    result.mutable_data()->mutable_challenge());
