...
public:
    ...
    ethash_mining_result_t PoWMine(const boost::multiprecision::uint256_t & blockNum,
                                   uint8_t difficulty,
...
private:
    ...
    ethash_return_value_t       EthashLightCompute(ethash_light_t & light, ethash_h256_t const &header_hash, uint64_t nonce);
    ethash_full_t               EthashFullNew(ethash_light_t &light, ethash_callback_t &callback);
    void                        EthashFullDelete(ethash_full_t & full);
    ethash_return_value_t       EthashFullCompute(ethash_full_t & full, ethash_h256_t const & header_hash, uint64_t nonce);
    ethash_mining_result_t      MineLight(ethash_light_t & light, ethash_h256_t const &header_hash, ethash_h256_t & difficulty);
    ethash_mining_result_t      MineFull(ethash_full_t & full, ethash_h256_t const &header_hash, ethash_h256_t & difficulty);
    std::vector<unsigned char>  ConcatAndhash(const std::array<unsigned char, UINT256_SIZE> &rand1,
                                              const std::array<unsigned char, UINT256_SIZE> & rand2,
                                              const boost::multiprecision::uint128_t & ipAddr,
                                              const PubKey & pubKey);
    bool                        VerifyLight(ethash_light_t & light,
                                            ethash_h256_t const & header_hash,
                                            uint64_t winning_nonce,
                                            ethash_h256_t & difficulty,
                                            ethash_h256_t & winning_result,
                                            ethash_h256_t & winning_mixhash);
    bool                        VerifyFull(ethash_full_t & full,
                                           ethash_h256_t const & header_hash,
                                           uint64_t winning_nonce,
                                           ethash_h256_t & difficulty,
                                           ethash_h256_t & winning_result,
