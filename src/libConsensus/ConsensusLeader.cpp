    ...
    ConsensusMessageType returnmsgtype, State nextstate)
{
    ...
    if (m_commitMap.at(backupID) == true)
    {
    ...
    // Check the commit
    if (commitPoint.Initialized() == false)
    {
                                               ...
                                               unsigned int offset)
{
    ...
    m_challenge = GetChallenge(m_messageToCosign, 0, m_lengthToCosign,
    ...
    if (m_responseCounter == m_numForConsensus)
    {
        ...
        if (result == true)
        {
            ...
            if (action == PROCESS_RESPONSE)
            {
                ...
                m_collectiveSig.Serialize(m_messageToCosign,
                                          m_messageToCosign.size());
                BitVector::SetBitVector(
                    m_messageToCosign, m_messageToCosign.size(), m_responseMap);
                m_lengthToCosign = m_messageToCosign.size();
    ...
    vector<unsigned char>& collectivesig, unsigned int offset)
{
    ...
    if (Schnorr::GetInstance().Verify(m_messageToCosign, 0, m_lengthToCosign,
    ...
    AnnouncementGeneratorFunc announcementGeneratorFunc)
{
    ...
    // ==================================
    vector<unsigned char> announcement_message
        = {m_classByte, m_insByte, ConsensusMessageType::ANNOUNCE};
