...
SafeLinuxTcpSocketServer::SafeLinuxTcpSocketServer(const std::string& ipToBind, const unsigned int &port) :
...
bool SafeLinuxTcpSocketServer::StartListening()
{
	if(!this->running)
	{
		...
		this->running = true;
		int ret = pthread_create(&(this->listenning_thread), NULL, SafeLinuxTcpSocketServer::LaunchLoop, this);
		if(ret != 0)
		{
...
bool SafeLinuxTcpSocketServer::StopListening()
{
	if(this->running)
	{
		this->running = false;
...
bool SafeLinuxTcpSocketServer::SendResponse(const string& response, void* addInfo)
{
	bool result = false;
...
void* SafeLinuxTcpSocketServer::LaunchLoop(void *p_data)
{
	pthread_detach(pthread_self());
	SafeLinuxTcpSocketServer *instance = reinterpret_cast<SafeLinuxTcpSocketServer*>(p_data);;
	instance->ListenLoop();
	return NULL;
}

void SafeLinuxTcpSocketServer::ListenLoop()
{
	int connection_fd = 0;
	...
	while(this->running)
	{
		if((connection_fd = accept(this->socket_fd, reinterpret_cast<struct sockaddr *>(&(connection_address)),  &address_length)) > 0)
		{
			...
			int ret = pthread_create(&client_thread, NULL, SafeLinuxTcpSocketServer::GenerateResponse, params);
			if(ret != 0)
			{
...
void* SafeLinuxTcpSocketServer::GenerateResponse(void *p_data)
{
	pthread_detach(pthread_self());
	struct GenerateResponseParameters* params = reinterpret_cast<struct GenerateResponseParameters*>(p_data);
	SafeLinuxTcpSocketServer *instance = params->instance;
...
bool SafeLinuxTcpSocketServer::WriteToSocket(const int& fd, const string& toWrite)
{
...
bool SafeLinuxTcpSocketServer::WaitClientClose(const int& fd, const int &timeout)
{
	bool ret = false;
	int i = 0;
...
int SafeLinuxTcpSocketServer::CloseByReset(const int& fd)
{
...
int SafeLinuxTcpSocketServer::CleanClose(const int& fd)
{
