...
/// Calculate SHA3-256 hash of the given input, returning as a 256-bit hash.
    inline h256 sha3(zbytesConstRef _input) { h256 ret; sha3(_input, ret.ref()); return ret; }
    inline SecureFixedHash<32> sha3Secure(zbytesConstRef _input) { SecureFixedHash<32> ret; sha3(_input, ret.writable().ref()); return ret; }

/// Calculate SHA3-256 hash of the given input, returning as a 256-bit hash.
    inline h256 sha3(zbytes const& _input) { return sha3(zbytesConstRef(&_input)); }
    inline SecureFixedHash<32> sha3Secure(zbytes const& _input) { return sha3Secure(zbytesConstRef(&_input)); }

/// Calculate SHA3-256 hash of the given input (presented as a binary-filled string), returning as a 256-bit hash.
    inline h256 sha3(std::string const& _input) { return sha3(zbytesConstRef(_input)); }
    inline SecureFixedHash<32> sha3Secure(std::string const& _input) { return sha3Secure(zbytesConstRef(_input)); }
...
/// Calculate SHA3-256 hash of the given input, possibly interpreting it as nibbles, and return the hash as a string filled with binary data.
    inline std::string sha3(std::string const& _input, bool _isNibbles) { return asString((_isNibbles ? sha3(fromHex(_input)) : sha3(zbytesConstRef(&_input))).asBytes()); }

/// Calculate SHA3-256 MAC
    inline void sha3mac(zbytesConstRef _secret, zbytesConstRef _plain, zbytesRef _output) { sha3(_secret.toBytes() + _plain.toBytes()).ref().populate(_output); }
