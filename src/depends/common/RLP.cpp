 ...
 * @date 2014
 */
// adapted from https://github.com/ethereum/cpp-ethereum/blob/develop/libdevcore/RLP.cpp
...
//			cdebug << "s: " << s << ", p: " << p << ", m_out.size(): " << m_out.size() << ", encodeSize: " << encodeSize << " (br: " << brs << ")";
            ...
            if (s < c_rlpListImmLenCount)
                m_out[p] = (zbyte)(c_rlpListStart + s);
            else if (c_rlpListIndLenZero + brs <= 0xff)
            {
                m_out[p] = (zbyte)(c_rlpListIndLenZero + brs);
                zbyte* b = &(m_out[p + brs]);
                for (; s; s >>= 8)
                    *(b--) = (zbyte)s;
            }
...
RLPStream& RLPStream::append(bigint _i)
{
    if (!_i)
        ...
        m_out.push_back((zbyte)_i);
    else
    {
        unsigned br = bytesRequired(_i);
        if (br < c_rlpDataImmLenCount)
            m_out.push_back((zbyte)(br + c_rlpDataImmLenStart));
        else
        {
            ...
            m_out.push_back((zbyte)(c_rlpDataIndLenZero + brbr));
            pushInt(br, brbr);
        }
        pushInt(_i, br);
    }
...
void RLPStream::pushCount(size_t _count, zbyte _base)
{
    ...
    m_out.push_back((zbyte)(br + _base));	// max 8 bytes.
    pushInt(_count, br);
}
