...
LevelDB::LevelDB(const string& dbName, const string& path,
    ...
    : m_dbName(dbName), m_subdirectory(subdirectory), m_db(nullptr) {
...
string LevelDB::Lookup(const boost::multiprecision::uint256_t& blockNum) const {
  string value;
  leveldb::Status s =
      m_db->Get(leveldb::ReadOptions(), blockNum.convert_to<string>(), &value);
...
string LevelDB::Lookup(const dev::bytesConstRef& key) const {
  string value;
  leveldb::Status s = m_db->Get(
      leveldb::ReadOptions(), ldb::Slice((char const*)key.data(), 32), &value);
  if (!s.ok()) {
    log_error(s);
    return "";
  }
...
std::shared_ptr<leveldb::DB> LevelDB::GetDB() { return this->m_db; }
...
int LevelDB::Insert(const string& key, const vector<unsigned char>& body) {
  return Insert(leveldb::Slice(key),
                leveldb::Slice(dev::bytesConstRef(&body[0], body.size())));
}
...
int LevelDB::DeleteDBForNormalNode() {
  ...
  if (this->m_subdirectory.size()) {
    boost::filesystem::remove_all(STORAGE_PATH + PERSISTENCE_PATH + "/" +
