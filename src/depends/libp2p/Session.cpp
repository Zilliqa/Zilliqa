...
bool Session::readPacket(uint16_t _capId, PacketType _t, RLP const& _r)
{
    ...
    {
        std::cout << "Exception caught in p2p::Session::interpret(): " << _e.what()
...
void Session::send(bytes&& _msg)
{
    ...
    if (!checkPacket(msg))
        std::cout << "INVALID PACKET CONSTRUCTED!";
...
void Session::drop(DisconnectReason _reason)
{
    ...
    if (socket.is_open())
        ...
        {
            boost::system::error_code ec;
            std::cout << "Closing " << socket.remote_endpoint(ec) << " (" << reasonOf(_reason)
                        << ")";
            socket.shutdown(boost::asio::ip::tcp::socket::shutdown_both, ec);
            socket.close();
        }
...
{
    std::cout << "Disconnecting (our reason: " << reasonOf(_reason) << ")";
...
void Session::doRead()
{
    ...
    ba::async_read(m_socket->ref(), boost::asio::buffer(m_data, h256::size),
        [this, self](boost::system::error_code ec, std::size_t length) {
            ...
            {
                std::cout << "header decrypt failed";
            ...
            {
                std::cout << "Exception decoding frame header RLP: " << _e.what();
//                << " "
            ...
            ba::async_read(m_socket->ref(), boost::asio::buffer(m_data, tlen),
                [this, self, hLength, hProtocolId, tlen](
                    ...
                    else
                    {
                        ...
                        if (!ok)
                            std::cout << "Couldn't interpret packet. " << RLP(r);
                    }
                    doRead();
                });
        });
...
bool Session::checkRead(std::size_t _expected, boost::system::error_code _ec, std::size_t _length)
{
    ...
    {
        std::cout << "Error reading: " << _ec.message();
        drop(TCPError);
        return false;
    }
    else if (_ec && _length < _expected)
    {
        std::cout << "Error reading - Abrupt peer disconnect: " << _ec.message();
    ...
    else if (_length != _expected)
    {
        ...
        std::cout << "Error reading - TCP read buffer length differs from expected frame size.";
