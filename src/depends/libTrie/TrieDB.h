...
class GenericTrieDB {
 public:
  ...
  GenericTrieDB(DB* _db, h256 const& _root,
  ...
  void open(DB* _db, h256 const& _root,
            Verification _v = Verification::Normal) {
    m_db = _db;
    setRoot(_root, _v);
  }
  ...
  void setRoot(h256 const& _root,
               Verification _v = Verification::Normal) const {
    m_root = _root;
  ...
  /// True if the trie is uninitialised (i.e. that the DB doesn't contain the
  /// root node).
  bool isNull() const { return !node(m_root).size(); }
  /// True if the trie is initialised but empty (i.e. that the DB contains the
  ...
  h256 const& root() const {
    if (node(m_root).empty()) {
      ...
      /*std::cout << "Returning root as " << ret << " (really " << m_root << ")"
       * << std::endl;*/
    }
    return m_root;
  }  // patch the root in the case of the empty trie. TODO: handle this
     // properly.
  ...
  class iterator {
   ...
   private:
    ...
    struct Node {
      ...
      void incrementChild() {
        child = child == 16 ? 0 : child == 15 ? 17 : (child + 1);
      }
  ...
  void descendKey(h256 const& _k, h256Hash& _keyMask, bool _wasExt,
                  std::ostream* _out, int _indent = 0) const {
    _keyMask.erase(_k);
    if (_k == m_root && _k == EmptyTrie)  // root allowed to be empty
      return;
    descendList(RLP(node(_k)), _keyMask, _wasExt, _out,
                _indent);  // if not, it must be a list
  }

  /// Used for debugging, scans the whole trie.
  void descendEntry(RLP const& _r, h256Hash& _keyMask, bool _wasExt,
                    std::ostream* _out, int _indent) const {
  ...
  /// Used for debugging, scans the whole trie.
  void descendList(RLP const& _r, h256Hash& _keyMask, bool _wasExt,
                   std::ostream* _out, int _indent) const {
  ...
  /// @warning This can be used to bypass the trie code. Don't use these unless
  /// you *really* know what you're doing.
  DB const* db() const { return m_db; }
  DB* db() { return m_db; }

 private:
  RLPStream& streamNode(RLPStream& _s, zbytes const& _b);

  std::string atAux(RLP const& _here, NibbleSlice _key) const;

  std::string getProof(RLP const& _here, NibbleSlice _key,
  ...
  // in: [V0, ..., V15, S] (DEL)
  // out: [V0, ..., V15, null]
  zbytes remove(RLP const& _orig);

  // in: [K1 & K2, V] (DEL) : nibbles(K1) == _s, 0 < _s <= nibbles(K1 & K2)
  // out: [K1, H] ; [K2, V] => H (INS)  (being  [K1, [K2, V]]  if necessary)
  zbytes cleve(RLP const& _orig, unsigned _s);

  // in: [K1, H] (DEL) ; H <= [K2, V] (DEL)  (being  [K1, [K2, V]] (DEL)  if
  // necessary) out: [K1 & K2, V]
  zbytes graft(RLP const& _orig);

  // in: [V0, ... V15, S] (DEL)
  // out1: [k{i}, Vi]    where i < 16
  // out2: [k{}, S]      where i == 16
  zbytes merge(RLP const& _orig, zbyte _i);
  ...
  // -- OR --
  // in: [k{i}K, V] (DEL)
  // out: [null ** i, H, null ** (16 - i)] ; [K, V] => H (INS)  (being [null **
  // i, [K, V], null ** (16 - i)]  if necessary)
  zbytes branch(RLP const& _orig);
  ...
  // This are semantically-aware node insertion functions that only kills when
  // the node's data is < 32 bytes. It can safely be used when pruning the trie
  // but won't work correctly for the special case of the root (which is always
  // looked up via a hash). In that case, use forceKillNode().
...
class HashedGenericTrieDB : private SpecificTrieDB<GenericTrieDB<_DB>, h256> {
 ...
 public:
  ...
  HashedGenericTrieDB(DB* _db, h256 _root,
                      Verification _v = Verification::Normal)
      : Super(_db, _root, _v) {}
  ...
  while (true) {
    ...
    if (m_trail.back().child == 255) {
      ...
      if (rlp.itemCount() == 2) {
        ...
        // TODO: do something different depending on how keyOfRLP compares to
        // k.mid(0, std::min(k.size(), keyOfRLP.size())); if == all is good -
        // continue descent. if > discard key and continue descent. if < discard
        // key and skip node.
        ...
        if (isLeaf(rlp)) {
          ...
          // Still data in key we're supposed to be looking for when we're at a
          // leaf. Go for next one.
          k.clear();
          m_trail.pop_back();
          continue;
        }
    ...
    for (;; m_trail.back().incrementChild()) {
      if (m_trail.back().child == 17) {
        ...
        if (m_trail.back().child == 16) {
          ...
          // fixed so that Node passed into push_back is constructed *before*
          // m_trail is potentially resized (which invalidates back and rlp)
...
std::string GenericTrieDB<DB>::atAux(RLP const& _here, NibbleSlice _key) const {
  ...
  if (itemCount == 2) {
    ...
    if (_key == k && isLeaf(_here))
      ...
      return atAux(
          _here[1].isList() ? _here[1] : RLP(node(_here[1].toHash<h256>())),
          _key.mid(k.size()));
    else
      // not us.
      return std::string();
...
template <class DB>
std::string GenericTrieDB<DB>::getProof(RLP const& _here, NibbleSlice _key,
                                        std::set<std::string>& nodes) const {
  LOG_MARKER();
  if (_here.isEmpty() || _here.isNull()) {
    // not found.
  ...
  if (itemCount == 2) {
    ...
    if (n.isEmpty()) {
      ...
      return getProof(n.isList() ? n : RLP(node(n.toHash<h256>())), _key.mid(1),
                      nodes);
    }
  }
}
...
void GenericTrieDB<DB>::mergeAtAux(RLPStream& _out, RLP const& _orig,
  ...
  // However, if may be a hash, in which case we deref and we know it is
  // removable.
  ...
  zbytes b = mergeAt(r, _k, _v, !isRemovable);
  streamNode(_out, b);
}
...
template <class DB>
zbytes GenericTrieDB<DB>::deleteAt(RLP const& _orig, NibbleSlice _k) {
  // The caller will make sure that the bytes are inserted properly.
  // - This might mean inserting an entry into m_over
  // We will take care to ensure that (our reference to) _orig is killed.
  ...
  if (_orig.itemCount() == 2) {
    ...
    if (_k.contains(k)) {
      ...
      if (!deleteAtAux(s, _orig[1], _k.mid(k.size()))) return zbytes();
      killNode(_orig);
      RLP r(s.out());
...
template <class DB>
bool GenericTrieDB<DB>::deleteAtAux(RLPStream& _out, RLP const& _orig,
                                    NibbleSlice _k) {
  zbytes b =
      _orig.isEmpty()
          ? zbytes()
          : deleteAt(_orig.isList() ? _orig : RLP(node(_orig.toHash<h256>())),
                     _k);
...
template <class DB>
RLPStream& GenericTrieDB<DB>::streamNode(RLPStream& _s, zbytes const& _b) {
  if (_b.size() < 32)
    _s.appendRaw(_b);
  else
