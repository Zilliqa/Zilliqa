    ...
    for(string name : programName )
    {
        for(pid_t pid : pids[name])
        {
            ...
            if(died[pid])
            {
                ...
                if(v == -1)
            	{
            		log<<"Error "<<pid<<" does not give proper value "<<endl;
            		return ;
            	}
                ...
                try
                {
                    ...
                    log<<"Params: "<<PubKey[pid]+" "+PrivKey[pid]+" "+Port[pid]+" "+to_string(v)+" "+Path[pid]+" "+name+" 2>&1"<<endl;
                    log<<"Output from shell...(if any)"<<endl<<endl;
                    log<<"\" "<<execute(restart_zilliqa+" "+PubKey[pid]+" "+PrivKey[pid]+" "+Port[pid]+" "+to_string(v)+" "+Path[pid]+" "+name+" 2>&1")<<" \""<<endl;
                    log<<endl;
                }
