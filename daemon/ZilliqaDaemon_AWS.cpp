...
SyncType getRestartValue(pid_t pid)
{
...
void initialize(unordered_map <string, vector<pid_t> > & pids, unordered_map <pid_t ,bool> & died, ofstream & log)
{
    ...
    for(auto v: programName)
    {
        ...
        if(tmp.size() > 0)
        {
            ...
            pids[v] = tmp;
            log<<"Process "<<v<<" exists in "<<pids[v].size()<<" instances"<<endl;
            log<<"Pids: ";
            for(auto i : pids[v])
            {
                log<<i<<" ";
...
void MonitorProcess(unordered_map <string, vector<pid_t> > & pids, unordered_map <pid_t, bool> & died, ofstream & log)
{
        ...
        for(pid_t pid : pids[name])
        {
           ...
           setIsDs(pid, log);
            ...
            if( w < 0)
            {
                if(errno == EPERM)
                {
                    log<<"Daemon does not have permission "<<"Name: "<<name<<" Id: "<<pid<<endl;
                }
                else if(errno == ESRCH)
                {
                    log<<"Process died "<<"Name: "<<name<<" Id: "<<pid<<endl;
                    died[pid] = true;
                }
                else
                {
                    log<<"Kill failed due to "<<errno<<" Name: "<<name<<"Id: "<<pid<<endl;
                }
            ...
            if(died[pid])
            {
                ...
                try
                {
                    ...
                    log<<"Params: "<<PubKey[pid]+" "+PrivKey[pid]+" "+Port[pid]+" "+to_string(v)+" "+Path[pid]+" 2>&1"<<endl;
                    log<<"Output from shell...(if any)"<<endl<<endl;
                    log<<"\" "<<execute(restart_zilliqa+" "+PubKey[pid]+" "+PrivKey[pid]+" "+Port[pid]+" "+to_string(v)+" "+Path[pid]+" 2>&1")<<" \""<<endl;
                ...
                for(pid_t i : tmp)
                {
                    auto it = find(pids[programName[0]].begin(), pids[programName[0]].end(), i);
                    if(it == pids[programName[0]].end())
                    {
                        died[i] = false;
                        pids[programName[0]].push_back(i);
                        log<<"Started new process "<<programName[0]<<" with PiD: "<<i<<endl;  
                    }
                }
                died.erase(pid);
                PrivKey.erase(pid);
