...
def CreateTempPersistence():
    bashCommand = (
    "rsync --recursive --inplace --delete -a "
    "persistence/otterTxAddressMappings "
    "persistence/otterTraces "
    "persistence/otterAddressNonceLookup "
    "tempBackupOtterDBs/persistence"
)
...
def backUp(curr_blockNum):
    ...
    os.chdir("tempBackupOtterDBs")
    logging.info("max processors  = "+ str(CPUS))
    pigz_command = "\"pigz -k -p {}\"".format(CPUS)
    command = 'tar --use-compress-program='+  pigz_command +' -cf ' + TESTNET_NAME +'OtterScanDB.tar.gz'+' persistence'
    try:
    ...
    os.system(awsCli() + " s3 cp " + TESTNET_NAME + "OtterScanDB.tar.gz " + AWS_PERSISTENCE_LOCATION + ".tar.gz")
    os.system(awsCli() + " s3 cp " + TESTNET_NAME + "OtterScanDB.tar.gz " + AWS_PERSISTENCE_LOCATION +  "-" + str(curr_blockNum) + ".tar.gz")
    os.remove(TESTNET_NAME + "OtterScanDB.tar.gz")
...
def main():
    ...
    last_backup_blockNum = None

    # create temp folder
    if os.path.exists('tempBackupOtterDBs/persistence'):
        shutil.rmtree('tempBackupOtterDBs/persistence')
    os.makedirs('tempBackupOtterDBs/persistence')
