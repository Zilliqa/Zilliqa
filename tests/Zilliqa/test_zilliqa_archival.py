...
def run_setup(numnodes, printnodes):
	...
	for x in range(0, numnodes):
		...
		shutil.copyfile('./tests/Zilliqa/zilliqa', testsubdir + '/archivezilliqa')

		st = os.stat(testsubdir + '/archivezilliqa')
		os.chmod(testsubdir + '/archivezilliqa', st.st_mode | stat.S_IEXEC)
...
def run_start():
	...
	for x in range(0, count):
		keypair = keypairs[x].split(" ")
                os.system('cd ' + LOCAL_RUN_FOLDER + testfolders_list[x] + '; echo \"' + keypair[0] + ' ' + keypair[1] + '\" > mykey.txt' + '; ulimit -n 65535; ulimit -Sc unlimited; ulimit -Hc unlimited; $(pwd)/archivezilliqa ' + ' --privk ' + keypair[1] + ' --pubk ' + keypair[0] + ' --address ' + '127.0.0.1' + ' --port '  + str(NODE_LISTEN_PORT + x) + ' > ./error_log_zilliqa 2>&1 &')
