...
BOOST_AUTO_TEST_CASE (test_curve_setup)
{
    ...
    if ((a != nullptr) && (b != nullptr) && (p != nullptr) && (h != nullptr))
    {
        ...
        BOOST_CHECK_MESSAGE(EC_GROUP_get_cofactor(schnorr.GetCurve().m_group.get(), h.get(), NULL) != 0, "EC_GROUP_get_cofactor failed");
        ...
        BOOST_CHECK_MESSAGE(strcmp(p_expected, p_actual.get()) == 0, "Wrong p generated");
        BOOST_CHECK_MESSAGE(strcmp(a_expected, a_actual.get()) == 0, "Wrong a generated");
        BOOST_CHECK_MESSAGE(strcmp(b_expected, b_actual.get()) == 0, "Wrong b generated");
        BOOST_CHECK_MESSAGE(strcmp(h_expected, h_actual.get()) == 0, "Wrong h generated");
    }
}
...
BOOST_AUTO_TEST_CASE (test_keys)
{
    ...
    BOOST_CHECK_MESSAGE(BN_cmp(keypair.first.m_d.get(), schnorr.GetCurve().m_order.get()) == -1, "Key generation check #1 failed");
    BOOST_CHECK_MESSAGE(BN_is_zero(keypair.first.m_d.get()) != 1, "Key generation check #2 failed");

    BOOST_CHECK_MESSAGE(EC_POINT_mul(schnorr.GetCurve().m_group.get(), P.get(), keypair.first.m_d.get(), NULL, NULL, NULL) != 0, "Key generation check #3 failed");
...
BOOST_AUTO_TEST_CASE (test_sign_verif)
{
    ...
    BOOST_CHECK_MESSAGE(BN_cmp(signature.m_s.get(), schnorr.GetCurve().m_order.get()) == -1, "Signature generation check #3 failed");
    BOOST_CHECK_MESSAGE(BN_is_zero(signature.m_s.get()) != 1, "Signature generation check #4 failed");

    // Verify the signature
    BOOST_CHECK_MESSAGE(schnorr.Verify(message_rand, signature, keypair.second) == true, "Signature verification (correct message) failed");
...
BOOST_AUTO_TEST_CASE (test_serialization)
{
    ...
    Signature signature;
    BOOST_CHECK_MESSAGE(schnorr.Sign(message, keypair.first, keypair.second, signature) == true, "Signing failed");
    BOOST_CHECK_MESSAGE(schnorr.Verify(message, signature, keypair.second) == true, "Signature verification failed");
    ...
    BOOST_CHECK_MESSAGE(keypair.first == privkey1, "PrivKey serialization check #1 failed");
    BOOST_CHECK_MESSAGE(keypair.second == pubkey1, "PubKey serialization check #1 failed");
    BOOST_CHECK_MESSAGE(signature == signature1, "Signature serialization check #1 failed");
    ...
    BOOST_CHECK_MESSAGE(schnorr.Sign(message_rand, keypair2.first, keypair2.second, signature2) == true, "Signing failed");
    BOOST_CHECK_MESSAGE(schnorr.Verify(message_rand, signature2, keypair2.second) == true, "Signature verification failed");
    ...
    BOOST_CHECK_MESSAGE(keypair.first == keypair2.first, "PrivKey serialization check #2 failed");
    BOOST_CHECK_MESSAGE(keypair.second == keypair2.second, "PubKey serialization check #2 failed");
