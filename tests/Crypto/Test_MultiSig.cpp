...
BOOST_AUTO_TEST_CASE (test_multisig)
{
    ...
    BOOST_CHECK_MESSAGE(challenge.Initialized() == true, "Challenge generation failed");
    ...
    for (unsigned int i = 0; i < nbsigners; i++)
    {
        ...
        BOOST_CHECK_MESSAGE(responses.back().Initialized() == true, "Response generation failed");
    }

    // Aggregate responses
    shared_ptr<Response> aggregatedResponse = MultiSig::AggregateResponses(responses);
    BOOST_CHECK_MESSAGE(aggregatedResponse != nullptr, "AggregateResponses failed");

    // Generate the aggregated signature
    shared_ptr<Signature> signature = MultiSig::AggregateSign(challenge, *aggregatedResponse);
    BOOST_CHECK_MESSAGE(signature != nullptr, "AggregateSign failed");

    // Verify the signature
    BOOST_CHECK_MESSAGE(schnorr.Verify(message_rand, *signature, *aggregatedPubkey) == true, "Signature verification (correct message) failed");
    ...
    BOOST_CHECK_MESSAGE(cp_copy == *aggregatedCommit, "CommitPoint operator= failed");

    Challenge challenge_copy;
    challenge_copy = challenge;
    BOOST_CHECK_MESSAGE(challenge_copy == challenge, "Challenge operator= failed");
...
BOOST_AUTO_TEST_CASE (test_serialization)
{
    ...
    shared_ptr<CommitPoint> aggregatedCommit = MultiSig::AggregateCommits(points);
    BOOST_CHECK_MESSAGE(aggregatedCommit != nullptr, "AggregateCommits failed");
    shared_ptr<CommitPoint> aggregatedCommit1 = MultiSig::AggregateCommits(points1);
    BOOST_CHECK_MESSAGE(*aggregatedCommit == *aggregatedCommit1, "Commit serialization failed");
    ...
    BOOST_CHECK_MESSAGE(challenge == challenge2, "Challenge serialization failed");
    tmp.clear();
    ...
    for (unsigned int i = 0; i < nbsigners; i++)
    {
        ...
        // Verify response
        BOOST_CHECK_MESSAGE(MultiSig::VerifyResponse(responses.at(i), challenge, pubkeys.at(i), points.at(i)) == true, "Verify response failed");
    }

    // Aggregate responses
    shared_ptr<Response> aggregatedResponse = MultiSig::AggregateResponses(responses);
    BOOST_CHECK_MESSAGE(aggregatedResponse != nullptr, "AggregateResponses failed");
    shared_ptr<Response> aggregatedResponse1 = MultiSig::AggregateResponses(responses1);
    BOOST_CHECK_MESSAGE(*aggregatedResponse == *aggregatedResponse1, "Response serialization failed");
