...
BOOST_AUTO_TEST_CASE(varyingOrderOfAddAccountCalls) {
  ...
  */
  ...
  AccountStore::GetInstance().AddAccount(address1, make_shared<Account>(2, 22));
  AccountStore::GetInstance().UpdateStateTrieAll();
  auto root2 = AccountStore::GetInstance().GetStateRootHash();
  ...
  */
  ...
  AccountStore::GetInstance().AddAccount(pubKey1, make_shared<Account>(2, 22));
  AccountStore::GetInstance().UpdateStateTrieAll();
  ...
  */
  ...
  AccountStore::GetInstance().AddAccount(pubKey2, make_shared<Account>(2, 22));
  AccountStore::GetInstance().UpdateStateTrieAll();
  ...
  */
  ...
  AccountStore::GetInstance().AddAccount(address3, make_shared<Account>(3, 33));
  AccountStore::GetInstance().UpdateStateTrieAll();
...
BOOST_AUTO_TEST_CASE(increaseBalance) {
  ...
  AccountStore::GetInstance().AddAccount(address1,
                                         make_shared<Account>(21, 211));
  AccountStore::GetInstance().UpdateStateTrieAll();
  auto root1 = AccountStore::GetInstance().GetStateRootHash();
...
BOOST_AUTO_TEST_CASE(transferBalance) {
  ...
  AccountStore::GetInstance().AddAccount(address2, make_shared<Account>(0, 1));
...
BOOST_AUTO_TEST_CASE(serialization) {
  ...
  // Increase coverage
  as.AddAccountDuringDeserialization(Address(), make_shared<Account>(),
                                     make_shared<Account>(), false, true);
  as.AddAccountDuringDeserialization(Address(), make_shared<Account>(),
                                     make_shared<Account>(), true, true);
}
...
BOOST_AUTO_TEST_CASE(temporaries) {
  ...
  BOOST_CHECK_EQUAL(false, AccountStore::GetInstance().UpdateAccountsTemp(
                               ...
                               true, Transaction(), tr, error_code));
...
void RunCFContract(
  ...
  AccountStore::GetInstance().AddAccountTemp(
      ownerAddr, make_shared<Account>(20000000000, nonce));
...

  {
    ...
    unique_lock<mutex> g(
        AccountStore::GetInstance().GetAccountWMutexTemp(contrAddr3, account3));
  ...
  {
    shared_ptr<Account> account4;
    unique_lock<mutex> g(
        AccountStore::GetInstance().GetAccountWMutexTemp(contrAddr4, account4));
...
void CheckRFContract(
  ...
  // Check the contract without invocation and retrieval
  {
    ...
    BOOST_CHECK_MESSAGE(stateJson3 == t_stateJson_3,
                        "StateJson3 doesn't match");
  ...
  // Check the contract with invocation and without retrieval
  {
    ...
    BOOST_CHECK_MESSAGE(stateJson4 == t_stateJson_4,
                        "StateJson4 doesn't match");
...
//   AccountStore::GetInstance().AddAccountTemp(address1,
//                                              make_shared<Account>(21, 211));
...
BOOST_AUTO_TEST_CASE(stateDelta) {
  ...
  BOOST_CHECK_MESSAGE(AccountStore::GetInstance().GetBalance(address1) == 0,
                      "address1 in AccountStore has balance before "
                      "deserializing delta, balance: "
                          << AccountStore::GetInstance().GetBalance(address1));
...
BOOST_AUTO_TEST_CASE(commitRevertible) {
  ...
  AccountStore::GetInstance().SerializeDelta();
  auto root0 = AccountStore::GetInstance().GetStateRootHash();
  LOG_GENERAL(INFO, "checker1");
  AccountStore::GetInstance().CommitTempRevertible();
  LOG_GENERAL(INFO, "checker2");
  auto root1 = AccountStore::GetInstance().GetStateRootHash();
  BOOST_CHECK_MESSAGE(
      AccountStore::GetInstance().GetBalance(address1) == 21,
  ...
  if (!SCILLA_ROOT.empty()) {
    ...
    shared_ptr<Account> t_account3 = make_shared<Account>();
    ...
    shared_ptr<Account> t_account4 = make_shared<Account>();
...
BOOST_AUTO_TEST_CASE(DiskOperation) {
  ...
  shared_ptr<Account> acc_ptr1;
  uint128_t balance1;
  {
    unique_lock<mutex> g(
        AccountStore::GetInstance().GetAccountWMutexTemp(address1, acc_ptr1));
    if (acc_ptr1) {
      balance1 = acc_ptr1->GetBalance();
    }
  }
