...
//     AccountStore::GetInstance().AddAccountTemp(senderAddr, {2000000000000,
//     nonce});
//   }
...
//     // uint64_t bnum =
//     ScillaTestUtil::GetBlockNumberFromJson(tests[0].blockchain); std::string
//     initStr = JSONUtils::GetInstance().convertJsontoStr(tests[0].init); bytes
//     data; uint64_t amount = 0; Address recipient; bytes code; if (deploy) {
//       data = bytes(initStr.begin(), initStr.end());
//       recipient = Address();
//       code = tests[0].code;
//       deployed = true;
...
//     Transaction tx(DataConversion::Pack(CHAIN_ID, 1), 0, contrAddr,
//     senders[i], stressAmount, PRECISION_MIN_VALUE, 20000, {}, stressData);
//     TransactionReceipt tr;
