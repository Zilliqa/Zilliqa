...
BOOST_AUTO_TEST_CASE(test_ethash_io_memo_file_match) {
  ...
  // let's make sure that the directory was created
  BOOST_REQUIRE(fs::is_directory(fs::path("./test_ethash_directory/")));
  // delete the full here so that memory is properly unmapped and FILE handler
  // freed
...
BOOST_AUTO_TEST_CASE(test_ethash_io_memo_file_size_mismatch) {
  ...
  // let's make sure that the directory was created
  BOOST_REQUIRE(fs::is_directory(fs::path("./test_ethash_directory/")));
  // and check that we get the size mismatch detected if we request diffferent
  // size
  BOOST_REQUIRE_EQUAL(
...
BOOST_AUTO_TEST_CASE(light_and_full_client_checks) {
  ...
  for (int i = 2; i < 32; i++) ethash_h256_set(&difficulty, i, 255);
  ...
  }
  {
    ...
    const std::string
        ...
        expected =
            "b1698f829f90b35455804e5185d78f549fcb1bdce2bee006d4d7e68"
  ...
  }
  {
    for (uint32_t i = 0; i < full_size / sizeof(node); ++i) {
      for (uint32_t j = 0; j < 32; ++j) {
        ...
        const std::string actual = bytesToHexString(
                              (uint8_t const*)&(full->data[j]), sizeof(node)),
                          expected = bytesToHexString(
                              (uint8_t const*)&expected_node, sizeof(node));
        BOOST_REQUIRE_MESSAGE(actual == expected,
                              "\ni: " << j << "\n"
                                      << "expected: " << expected.c_str()
                                      << "\n"
...
BOOST_AUTO_TEST_CASE(ethash_full_new_when_dag_exists_with_wrong_size) {
  ...
  // then create new DAG, which should detect the wrong size and force create a
  // new file
  ethash_light_t light = ethash_light_new_internal(cache_size, &seed);
  BOOST_ASSERT(light);
  ethash_full_t full = ethash_full_new_internal("./test_ethash_directory/",
...
BOOST_AUTO_TEST_CASE(test_incomplete_dag_file) {
  ...
  ethash_full_t full =
      ethash_full_new_internal("./test_ethash_directory/", seed, full_size,
                               light, test_full_callback_create_incomplete_dag);
  BOOST_ASSERT(!full);
  UNUSED(full);
  FILE* f = NULL;
