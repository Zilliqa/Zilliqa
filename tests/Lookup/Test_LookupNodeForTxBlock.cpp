...
void SendDSBlockFirstToMatchDSBlockNum(Peer& lookup_node) {
  ...
  zil::p2p::GetInstance().SendMessage(nullptr, lookup_node, dsblockmsg);
}
...
BOOST_AUTO_TEST_CASE(testTxBlockStoring) {
  ...
  zil::p2p::GetInstance().SendMessage(nullptr, lookup_node, txblockmsg);
}
...
BOOST_AUTO_TEST_CASE(testTxBlockRetrieval) {
  ...
  if (!Messenger::SetLookupGetTxBlockFromSeed(
    ...
    zil::p2p::GetInstance().SendMessage(nullptr, lookup_node,
                                        getTxBlockMessage);
  }
}
