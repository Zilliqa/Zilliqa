...
describe("Calling " + METHOD, function () {
  describe("When on Zilliqa network", function () {
    ...
    it("should have valid structure in response", async function () {
      ...
      await sendJsonRpcRequest(METHOD, 2, [transactionHash],
        (result, status) => {
          logDebug(result);
          ...
          assert.equal(
            parseInt(result.result.value, 16),
            1_000_000,
            "Is not equal to " + 1_000_000
          );
        }
      );
    })
    ...
    it("should return null when no transaction was found", async function () {
      ...
      await sendJsonRpcRequest(METHOD, 1, [INVALID_HASH],
        (result, status) => {
