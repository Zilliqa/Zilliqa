...
describe(`Calling ${METHOD} #parallel`, function () {
  ...
  it("should return the raw transaction response for payment txn @block-1", async function () {
    let amount = 10_000;
  ...
  it("should return the raw transaction response for contract txn @block-1", async function () {
    ...
    await sendJsonRpcRequest(METHOD, 1, [transactionHash], (result, status) => {
      ...
      assert.equal(
        ...
        "Is not equal to " + contract.address.toUpperCase()
      );
      ...
      //logs
      assert.isString(result.result.logs[0].transactionIndex, "Is not a string");
      assert.match(result.result.logs[0].transactionIndex, /^0x/, "Should be HEX starting with 0x");

      //data
      assert.isString(result.result.logs[0].data, "Is not a string");
      assert.match(result.result.logs[0].data, /^0x/, "Should be HEX starting with 0x");

      //logIndex
      assert.isString(result.result.logs[0].logIndex, "Is not a string");
