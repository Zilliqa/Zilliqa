...
describe("RFC75 ScillaCallComplex", function () {
    ...
    beforeEach(async function () {
        solidityContract = await parallelizer.deployContract("ScillaCallComplex");
    ...
    it("It should see updates states withing single solidity function call", async function () {
        const CALL_MODE = 0;
        await expect(solidityContract.SetAndGet(scillaContractAddress, "setValue", CALL_MODE, solidityContract.address, VAL))
            .not.to.be.reverted;
    });
