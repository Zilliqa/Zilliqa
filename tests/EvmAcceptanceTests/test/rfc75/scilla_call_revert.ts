...
describe("RFC75 ScillaCallRevert", function () {
    ...
    it("It should be reverted when one of sent messages from scilla contract is sent to evm contract implementing scila receiver", async function () {
        const CALL_MODE = 0;
        await expect(
            solidityContract.callScilla(scillaContractAddress, "callWithTwoRecipients", CALL_MODE, VAL, solidityContract.address, solidityContractReceiver.address)
        ).to.be.reverted;
    ...
    it("It should be reverted when one of sent messages from scilla contract has _EvmCall tag", async function () {
        const CALL_MODE = 0;
        await expect(
            solidityContract.callScilla(scillaContractAddress, "callWithTwoRecipientsEvmCall", CALL_MODE, VAL, solidityContract.address, solidityContract.address)
        ).to.be.reverted;
        expect(await scillaContract.value()).to.be.eq(0);
    });
    ...
    it("It should not be reverted when messages sent from scilla contract are sent to other scilla contracts", async function () {
        const CALL_MODE = 0;
        await expect(
            solidityContract.callScilla(scillaContractAddress, "callWithTwoRecipients", CALL_MODE, VAL, scillaContractAddress, scillaContractAddress)
        ).not.to.be.reverted;
    ...
    it("It should be reverted when message sent in chain eventually triggers a message sent to evm contract with scillaReceiver", async function () {
        const CALL_MODE = 0;
        await expect(
            solidityContract.callScillaChain(scillaContractAddress, "callAndForward", CALL_MODE, VAL, scillaContractAddress, solidityContractReceiver.address, solidityContract.address)
        ).to.be.reverted;
        expect(await scillaContract.value()).to.be.eq(0);
    });
    ...
    it("It should be reverted when message sent in chain eventually triggers a message sent to evm contract with EvmCall tag", async function () {
        const CALL_MODE = 0;
        await expect(
            solidityContract.callScillaChain(scillaContractAddress, "callAndForwardToEvmCall", CALL_MODE, VAL, scillaContractAddress, solidityContract.address, solidityContract.address)
        ).to.be.reverted;
        expect(await scillaContract.value()).to.be.eq(0);
    });
