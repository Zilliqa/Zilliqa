...
function MyReporter(runner) {
  ...
  function resultsString(delim: string): string {
    return clc.bold.white(`Total: ${totalTests()}`) + delim +
        clc.bold.green(`Passed: ${passes}`) + delim +
        clc.bold.red(`Failed: ${failures}`) + delim +
        clc.bold.yellow(`Timed out: ${timeouts}`) + delim +
